...
public partial class MainWindow : Window
{
    ...
    public MainWindow(AutoServiceManager manager)
    {
        Manager = manager;
        InitializeComponent();
        BuildUi();
    ...
    public MainWindow()
    {
        Manager = new AutoServiceManager();
        InitializeComponent();
    }
    ...
    private Control BuildCustomersTab()
    {
        ...
        form.Children.Add(RowButtons(
            ("Create", () => { Manager.AddCustomer(_customerName.Text ?? "", _customerPhone.Text ?? "", _customerEmail.Text ?? "", _customerAddress.Text ?? ""); ClearCustomerForm(); RefreshAll(); }
        ),
            ("Save", () => { if (_customerList.SelectedItem is Customer c) { Manager.UpdateCustomer(c, _customerName.Text ?? "", _customerPhone.Text ?? "", _customerEmail.Text ?? "", _customerAddress.Text ?? ""); RefreshAll(); } }
        ),
            ("Delete", () => { if (_customerList.SelectedItem is Customer c) { _customerList.ItemsSource = null; Manager.DeleteCustomer(c); ClearCustomerForm(); RefreshAll(); } }
        )));
        ...
        _customerList.SelectionChanged += (_, _) =>
        {
            if (_customerList.SelectedItem is Customer c)
            {
                ...
                _customerPhone.Text = c.ContactDetails.Phone;
                _customerEmail.Text = c.ContactDetails.Email;
                _customerAddress.Text = c.ContactDetails.Address;
            }
        };
    ...
    private Control BuildCarsTab()
    {
        ...
        form.Children.Add(RowButtons(
            ("Create", () => { Manager.AddCar(_carCustomer.SelectedItem as Customer, _carMake.Text ?? "", _carModel.Text ?? "", Int(_carYear.Text), _carVin.Text ?? "", Int(_carMileage.Text), _carLicense.Text ?? ""); ClearCarForm(); RefreshAll(); }
        ),
            ("Save", () => { if (_carList.SelectedItem is Car car) { Manager.UpdateCar(car, _carCustomer.SelectedItem as Customer, _carMake.Text ?? "", _carModel.Text ?? "", Int(_carYear.Text), _carVin.Text ?? "", Int(_carMileage.Text), _carLicense.Text ?? ""); RefreshAll(); } }
        ),
            ("Delete", () => { if (_carList.SelectedItem is Car car) { _carList.ItemsSource = null; Manager.DeleteCar(car); ClearCarForm(); RefreshAll(); } }
        )));
        Grid.SetColumn(form, 0);
        grid.Children.Add(form);
    ...
    private Control BuildOrdersTab()
    {
        ...
        _orderStatus = new ComboBox { ItemsSource = Enum.GetNames(typeof(OrderStatus)), SelectedIndex = 0 };
        _orderPayment = new ComboBox { ItemsSource = Enum.GetNames(typeof(PaymentType)), SelectedIndex = 0 };
        _orderProblem = Box("Problem description");
        _orderCost = Box("Cost (Auto-calculated)");
        ...
        form.Children.Add(RowButtons(
            ("Create", () => {
                Enum.TryParse(_orderStatus.SelectedItem?.ToString(), out OrderStatus stat);
                Enum.TryParse(_orderPayment.SelectedItem?.ToString(), out PaymentType pay);
                Manager.CreateOrder(_orderCustomer.SelectedItem as Customer, _orderCar.SelectedItem as Car, _orderProblem.Text ?? "", _orderMechanic.SelectedItem as Mechanic, stat, pay);
                ClearOrderForm(); RefreshAll();
            }
        ),
            ("Save", () => {
                if (_orderList.SelectedItem is RepairOrder o)
                {
                    Enum.TryParse(_orderStatus.SelectedItem?.ToString(), out OrderStatus stat);
                    Enum.TryParse(_orderPayment.SelectedItem?.ToString(), out PaymentType pay);
                    Manager.UpdateOrder(o, _orderCustomer.SelectedItem as Customer, _orderCar.SelectedItem as Car, _orderProblem.Text ?? "", _orderMechanic.SelectedItem as Mechanic, stat, pay);
                    RefreshAll();
                }
            }
        ),
            ("Delete", () => { if (_orderList.SelectedItem is RepairOrder o) { _orderList.ItemsSource = null; Manager.Orders.Remove(o); Manager.SaveAll(); ClearOrderForm(); RefreshAll(); } }
        )));
        ...
        {
            if (_orderList.SelectedItem is RepairOrder o) FillOrder(o);
        };
    ...
    private Control BuildPartsTab()
    {
        ...
        form.Children.Add(RowButtons(
            ("Add", () => { Manager.AddPart(_partName.Text ?? "", _partArticle.Text ?? "", Decimal(_partPrice.Text), Int(_partStock.Text)); ClearPartForm(); RefreshAll(); }
        ),
            ("Save", () => { if (_partList.SelectedItem is Part p) { Manager.UpdatePart(p, _partName.Text ?? "", _partArticle.Text ?? "", Decimal(_partPrice.Text), Int(_partStock.Text)); RefreshAll(); } }
        ),
            ("Delete", () => { if (_partList.SelectedItem is Part p) { _partList.ItemsSource = null; Manager.DeletePart(p); ClearPartForm(); RefreshAll(); } }
        )));
        Grid.SetColumn(form, 0);
        grid.Children.Add(form);
    ...
    private Control BuildMechanicsTab()
    {
        ...
        form.Children.Add(RowButtons(
            ("Create", () => { Manager.AddMechanic(_mechanicName.Text ?? "", _mechanicSpec.Text ?? "", Decimal(_mechanicRate.Text)); ClearMechanicForm(); RefreshAll(); }
        ),
            ("Save", () => { if (_mechanicList.SelectedItem is Mechanic m) { Manager.UpdateMechanic(m, _mechanicName.Text ?? "", _mechanicSpec.Text ?? "", Decimal(_mechanicRate.Text)); RefreshAll(); } }
        ),
            ("Delete", () => { if (_mechanicList.SelectedItem is Mechanic m) { _mechanicList.ItemsSource = null; Manager.DeleteMechanic(m); ClearMechanicForm(); RefreshAll(); } }
        )));
    ...
    private StackPanel RowButtons(params (string Text, Action Handler)[] buttons)
    {
        var row = new StackPanel { Orientation = Orientation.Horizontal, Spacing = 8, Margin = new Avalonia.Thickness(0, 8, 0, 0) };
        foreach (var b in buttons)
            row.Children.Add(Button(b.Text, (_, _) => b.Handler()));
        return row;
    }
    ...
    private void FillOrder(RepairOrder o)
    {
        ...
        _orderStatus.SelectedItem = o.Status.ToString();
        _orderPayment.SelectedItem = o.PaymentMethod.ToString();
        _orderProblem.Text = o.ProblemDescription;
        _orderCost.Text = o.CalculateTotalCost().ToString("C");
